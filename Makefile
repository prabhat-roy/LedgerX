...
help:
	...
	@echo "  lint           Lint all source (Go, Java, Kotlin, Rust, Python, Scala, Haskell, Node, TS)"
...
tools-deploy:  ## Deploy every OSS tool registered for LedgerX via ArgoCD
	kubectl apply -f gitops/argocd/applicationsets/all-tools-applicationset.yaml
	argocd appset get ledgerx-tools --grpc-web

tools-status:  ## Show sync status of every registered tool
	argocd app list -l "project=ledgerx-tools" --grpc-web
