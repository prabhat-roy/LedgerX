# AI_PLAN.md — LedgerX (Banking, Capital Markets & Wealth Platform)
...
- **Conversational client experience** — robo-advisor, chatbots — across
  retail, premier, and SME segments.
- **Regulatory-reporting NLP** — the volume of MAR / MiFID II / SEBI /
...
`ledger-architect` — researches AI tooling, proposes services, on-boards
...
Respond → Upgrade → Report. Plus a **model-risk-management (MRM) gate**
...
```
ai-platform/
├── cluster: ledger-ai-{aws,gcp,azure}    ← cloud GPU pool, PCI-DSS-segmented
├── namespace: ledger-ai-control           ← Paperclip
├── namespace: ledger-ai-agents            ← OpenClaw runtime
├── namespace: ledger-ai-sandbox           ← NemoClaw — strict financial guardrails
├── namespace: ledger-ai-models            ← vLLM, Ollama, LiteLLM, Triton
├── namespace: ledger-ai-data              ← Qdrant, Weaviate, MinIO, MLflow, Feast
├── namespace: ledger-ai-obs               ← Langfuse, Phoenix
└── namespace: ledger-ai-pipelines         ← Argo Workflows + Kubeflow
```
...
| Sandbox | **NemoClaw** | NeMo Guardrails — financial-safe |
...
- Every prompt + completion → Langfuse + Paperclip with **customer-ID hashed**.
...
| 5 | Per-service Tier-2 agents (payments → cards → aml first) |
...
- **Cloud infra**: $7,500 – $11,000 / month per primary cloud (PCI premium)
