...
spark = SparkSession.builder.appName("ledgerx-segment").getOrCreate()
df = spark.read.format("iceberg").load("ledgerx_mart.user_features")
...
preds.select("user_id","prediction").writeTo("ledgerx_mart.user_segments").using("iceberg").createOrReplace()
